...
bool SDKMinimalClient::isRawErgoCombined = false;
...
ClientReturnCode SDKMinimalClient::InitializeSDK()
{
	...
	ClientLog::print("[1] Stream raw skeleton data (joint positions)");
	ClientLog::print("[2] Stream ergonomic data (joint angles)");
	ClientLog::print("[3] Stream raw skeleton data (thumb only) and ergonomic data (index and middle only)");

	std::string dataStreamTypeInput;
	std::cin >> dataStreamTypeInput;

	// Register client selected data stream callback
	ClientReturnCode t_CallBackResults;
	switch (dataStreamTypeInput[0])
	{
	case '1':
		t_CallBackResults = RegisterRawSkeletonStreamCallback();
		break;
	case '2':
		t_CallBackResults = RegisterErgonomicStreamCallback();
		break;
	case '3':
		t_CallBackResults = RegisterAllCallbacks();
		isRawErgoCombined = true;
		break;
	default:
		ClientLog::print("Invalid input, try again");
		return ClientReturnCode::ClientReturnCode_FailedToInitialize;
	...
	t_VUH.view = AxisView::AxisView_YFromViewer;
	...
	const SDKReturnCode t_CoordinateResult = CoreSdk_InitializeCoordinateSystemWithVUH(t_VUH, true); // True for world, false for local
...
/// @brief Used to register all the stream callbacks, in this case they are the raw skeleton data stream (coordinates and orientations of joints) and ergonimic data stream (joint angles)
/// Callbacks that are registered functions that get called when a certain 'event' happens, such as data coming in.
/// All of these are optional, but depending on what data you require you may or may not need all of them. For this example we only implement the raw skeleton data and ergonomic data.
...
/// @brief Used to register only raw skeleton data stream callback (coordinates and orientations of joints)
ClientReturnCode SDKMinimalClient::RegisterRawSkeletonStreamCallback()
{
	const SDKReturnCode t_RegisterRawSkeletonCallbackResult = CoreSdk_RegisterCallbackForRawSkeletonStream(*OnRawSkeletonStreamCallback);
	if (t_RegisterRawSkeletonCallbackResult != SDKReturnCode::SDKReturnCode_Success)
	{
...
/// @brief Used to register only ergonimic data stream callback (joint angles)
ClientReturnCode SDKMinimalClient::RegisterErgonomicStreamCallback()
{
	const SDKReturnCode t_RegisterErgonomicsCallbackResult = CoreSdk_RegisterCallbackForErgonomicsStream(*OnErgonomicsCallback);
	if (t_RegisterErgonomicsCallbackResult != SDKReturnCode::SDKReturnCode_Success)
	{
...
void SDKMinimalClient::Run()
{
	...
	// set the hand motion mode of the RawSkeletonStream. This is optional and can be set to any of the HandMotion enum values. Default = HandMotion_None
	// HandMotion_Auto will make it move based on available tracking data. If none is available IMU rotation will be used.
	// HandMotion_Tracker: Fixed coordinate system
	// HandMotion_IMU: Coordinate system transforms along with the rotation of palm

	
	const SDKReturnCode t_HandMotionResult = CoreSdk_SetRawSkeletonHandMotion(HandMotion_Tracker);
	...
	while (m_Running)
	{
		...
		std::this_thread::sleep_for(std::chrono::milliseconds(1000)); // Roughly 30fps, good enough to show the results, but too slow to retrieve all data.
...
void SDKMinimalClient::PrintRawSkeletonNodeInfo()
{
	if ((m_RawSkeleton == nullptr || m_RawSkeleton->skeletons.size() == 0) || m_PrintedNodeInfo)
	{
		if (m_RawSkeleton->skeletons.size() != 0 && m_RawSkeleton->skeletons[0].nodes.size() != 0) {
			...
			// prints the position and rotation of the selected node in the selected skeleton
			// node 4 is the fingertip of the index
			ManusVec3 t_Pos = m_RawSkeleton->skeletons[0].nodes[4].transform.position;
			ManusQuaternion t_Rot = m_RawSkeleton->skeletons[0].nodes[4].transform.rotation;
...
void SDKMinimalClient::OnErgonomicsCallback(const ErgonomicsStream* const p_Ergo)
{
	...
	if (s_Instance)
	{
		for (uint32_t i = 0; i < p_Ergo->dataCount; i++) /*dataCount refers to number fo gloves*/
		{	
			if (p_Ergo->data[i].isUserID)continue;

			ErgonomicsData* t_Ergo = nullptr;
			//m_FirstLeftGloveID to be set in header file
